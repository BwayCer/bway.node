...
function ( log, _supportJS ) {
     ...
     * @param {Function} [factory] - 完成通知。
     */
    internWaiter.prototype.runPass = function ( strId, arrDeps, fnFactory ) {
        strId += '?id=' + Math.random().toString().substr( -7 );
        fnFactory = typeof fnFactory === 'function' ? fnFactory : null;

        var _waiting = this._waiting;
        var insInfoForm = new _infoFormForPass( this, strId, arrDeps, fnFactory );

        if ( insInfoForm.remainder ) {
            this._waitingCount++;
     ...
     * @param {Function} [factory] - 完成通知。
     */
    function _infoFormForPass( insSelf, strId, arrDeps, anyFactory ) {
        this.self = insSelf;
        this.stage = 'receive';
        this.timestamp = +new Date();
    ...
    _infoFormForPass.prototype.pass = function ( arrDeps ) {
        var p, len;
        var id, idInfo, insDepInfoForm, newFileInfo;
        var insSelf = this.self;
        var _cache = insSelf._cache;
        var _waiting = insSelf._waiting;
        var deps = this.deps = [];
        ...
        for ( p = 0, len = arrDeps.length; p < len ; p++ ) {
            ...
            if ( id in _cache ) {
                depInfo[ id ] = 'created';
                depStore[ id ] = _cache[ id ];
            } else if ( id in _waiting ) {
                this.remainder++;
                insDepInfoForm = depInfo[ id ] = _waiting[ id ];
                insDepInfoForm.replyList.push( this );
            } else {
                this.remainder++;
                depInfo[ id ] = 'needRequest';
                idInfo[ 3 ] = {};
                arrNeedRequest.push( idInfo );
            }
        }
     ...
     * @func _submit
     */
    _infoFormForPass.prototype._submit = function () {
        this.stage = 'create';
        ...
        if ( factory ) factory.apply( null, this._getPushArgs() );
     ...
     */
    internWaiter.prototype.request = function ( objQueryString, strId, fnDone ) {};
     ...
     * @param {?*} factory - 工廠。
     */
    internWaiter.prototype.finish = function ( strId, anyModule, txt, anyDeps, anyDepPkg, anyFactory ) {};
