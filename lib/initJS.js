...
void function () {
    ...
    var classLog, log, cacher, internWaiter;
    ...
    internWaiter    = args[ idx++ ]( log, _supportJS, cacher );

    args[ idx++ ]( log, internWaiter, classLog, cacher );
}(
// defaultMsgTable
{
...
// logMsgTable
{
    ...
    internWaiter_notAllowedEmptyId: 'The "id" is not allowed empty value.',
...
// internWaiter
function ( log, _supportJS, _cacher ) {
    var _emptyEnum = _supportJS.emptyEnum;
    ...
    function internWaiter( objPreSource ) {
        this._cacher = _import( new _cacher(), objPreSource );
        this._waiting = new _cacher();
     ...
     * @func _import
     * @param {module:initJS.cacher} cacher - 快取器。
     * @param {Object} source - 來源物件。
     * @return {module:initJS.cacher} 回傳第一項參數。
     */
    function _import( insCacher, objSource ) {
        if ( !objSource ) return insCacher;
     ...
     * @return {Function}
     */
    _getChannel.toBind = Function( 'insSelf',
        ...
        +   'else return insSelf.run( args[ 0 ], args[ 1 ], args[ 2 ], fileInfo );'
        + '};'
    );
    ...
    internWaiter.prototype.run = function ( strId, anyDeps, anyFactory, objFileInfo ) {
        ...
        var id = idInfo[ 0 ];
        var _cacher = this._cacher;
        var _waiting = this._waiting;
        var bisWaiting = _waiting.has( id );

        if ( bisWaiting ) insInfoForm = _waiting.pick( id );
        else  {
            // 以手動訂單更新物件
            if ( _cacher.has( id ) ) _cacher.rm( id );
        ...
        if ( !bisWaiting && insInfoForm.remainder ) {
            _waiting.add( id, insInfoForm );
            _updateTimer( this, true );
        }
    };
    ...
    _updateTimer._timerFunc = function ( insSelf ) {
        ...
        if ( !insSelf._waiting._index.length ) return;
    ...
    _infoForm.prototype._handleDeps = function ( arrDeps, insSelf, objDepInfo, objDepStore ) {
        ...
        for ( p = 0, len = arrDeps.length; p < len ; p++ ) {
            ...
            if ( _cacher.has( depId ) ) {
                objDepInfo[ depId ] = 'created';
                objDepStore[ depId ] = _cacher.pick( depId );
            } else if ( _waiting.has( depId ) ) {
                this.remainder++;
                insDepInfoForm = objDepInfo[ depId ] = _waiting.pick( depId );
    ...
    _infoForm.prototype._requestDep = function ( arrDepIdInfo, objNewFileInfo ) {
        ...
        insSelf._waiting.add( depId, insDepInfoForm );
    ...
    _infoForm.prototype._submit = function () {
        ...
        insSelf._cacher.add( id, module );
        insSelf._waiting.rm( id );
     ...
     * @func _parseId
     * @param {String} id - 識別碼。
     * @throws {Error} 識別碼不允許為空值。 (21, internWaiter_notAllowedEmptyId)
    ...
    _infoForm.prototype._parseId = function ( strId ) {
        if ( !strId ) throw Error( this.catchErr( 'internWaiter_notAllowedEmptyId' ) );
    ...
    function _stopProcess( insSelf, strMsgCode ) {
        var _waiting = insSelf._waiting;
        var _waitingIndex = _waiting._index;

        if ( !_waitingIndex.length ) return;

        var key, insInfoForm;
        var _waitingCache = _waiting._cache;
        var waitingInfo = [];

        while ( _waitingIndex.length ) {
            key = _waitingIndex[ 0 ];
            insInfoForm = _waitingCache[ key ];
...
function yeseed( log, internWaiter, classLog, cacher ) {
    var insWaiter = new internWaiter();
    var order = insWaiter.order;

    order.cache = insWaiter._cacher._cache;
    order( '/bway/classLog', classLog );
    order( '/bway/cacher', cacher );
    order( '/bway/log', log );
    order( '/bway/internWaiter', internWaiter );
