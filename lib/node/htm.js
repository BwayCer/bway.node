...
order( [
    ...
    void function () {
         ...
         */
        _createPositioning = _isFormat
            ? function createPositioning( strName, preCobj, strLineBreakCharacter ) {
                var strAns = '${ param.' + strName + ' || "' + ( preCobj || '' ) + '" }';

                switch ( strLineBreakCharacter ) {
            ...
            : function createPositioning( strName, preCobj ) {
                var strAns = '${ param.' + strName + ' || "' + ( preCobj || '' ) + '" }';
    ...
    void function () {
         ...
         * @param {String} [lineBreakCharacter] - 換行符（純文本模式專用，若無預設物件則需填 `null`）。
    ...
    void function () {
        ...
        function layoutTxt( strVal ) {
            ...
            _txtHtml += _gapCharacter + _gapCharacter + _matchPositioning( strVal );
         ...
         * @memberof module:htm.
         * @func style
         * @param {(Object|Function)} stylesheet - 爪哇腳本的樣式表。
         * 見 {@link module:htf._tag|htf._tag} 。
         * @return {(module:htm.head|module:htm.body)} 視當時的 `this` 而定。
         */
        function style( anyStylesheet ) {
            _parseWrapProcedure( this );

            _txtHtml += _gapCharacter + _gapCharacter
                + _increaseIndent( '        ', _tagStyle( anyStylesheet ) );
         ...
         * `/\{\{(?:([SN]{2})_)?([$_0-9A-Za-z]+)\}\}/g` 。
         * <br>
         * 換行符 `SN_` 的搭配組合是用以決定格式化下換行符的使用方式，`N` 換行，`S` 不變。
         *
         * @memberof module:htm~
         * @var {RegExp} _regexPositioning
         */
