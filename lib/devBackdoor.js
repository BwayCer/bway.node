...
void function ( devBackdoor ) {
    if ( typeof module === "object" ) module.exports = devBackdoor;
    else if ( typeof window === "object" ) window.devBackdoor = devBackdoor;
}(
function ( ion ) {
    var log = ion.log;
    var jspi = ion.jspi;
    ...
    if ( log ) {
        ...
        log.catchErr = function ( fnLandmine, classErr, numCode, strMsgCode, strShowMsg ) {
            var err, actual, errInfo, checkBySelf, rightType, rightCode, rightMsgCode;
            ...
            err = Error( strShowMsg + '（' + actual.toString() + '）' );
