 ...
 * @param {...?module:htParse~_CreateElement} childNode - 若為 `null` 時則忽略。
 *
 * @example
 * new _CreateNodeList(
...
function HTParse(CreateElement, CreateNodeList) {
    ...
    void function (HTParseSelf, CreateElement, CreateNodeList) {
        ...
        HTParseSelf.tag = function tag() {
            var fnT = _t.bind(null);
            fnT.singleTag = tag.singleTag;
            fnT.nodeList = tag.nodeList;
            return fnT;
        };
        ...
        HTParseSelf.tag.nodeList = function nodeList() {
            return new CreateNodeList(arguments);
        };
...
console.log(htParse.tagHandle(function (t) {
    t('div', {id: 'postForm'},
        function () {
            console.log(this.id);
        },
