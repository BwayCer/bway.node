...
order( [
    ...
    function pureText() {
         ...
         */
        _createElement.prototype.appendChild = isFormat
            ? function ( anyChild ) {
        ...
        var _hasChildPreCharacter = isFormat ? '\n' : '';

        /**
         * @memberof module:htf~_docClass._createElement~
         * @var {Array} _singleTag_pureText - 單標籤。
         * 目前值為： `meta`, `br`, `hr`, `link`, `img`, `input`
         */
        var _singleTag = [ 'meta', 'br', 'hr', 'link', 'img', 'input' ];
        ...
        _createElement.prototype.elt = function elt() {
            ...
            if ( ~_singleTag.indexOf( tagName ) ) strAns += ' />';
            else {
                strAns += '>';

                item = this._childs;
                if ( item ) strAns += _hasChildPreCharacter + item;
    ...
    void function () {
         ...
         */
        _tag.easyTag = function easyTag( strTagName ) {
            var insTag = new _createElement( strTagName );
